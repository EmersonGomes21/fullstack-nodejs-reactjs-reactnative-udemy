...
const TaskValidation = async (req, res, next) => {
    ...
    if (!macaddress) return res.status(400).json({ error: 'macaddress é obrigatório!' });
    ...
    else if (!description) return res.status(400).json({ error: 'A descrição é obrigatória!' });
        ...
        if (req.params.id) {
            ...
            if (isPast(new Date(when))) return res.status(400).json({ error: 'Escolha uma data futura!' });
